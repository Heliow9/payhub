...
const dateBr=(v:any)=>{if(!v)return'—';const m=/^(\d{4})-(\d{2})-(\d{2})/.exec(String(v));return m?`${m[3]}/${m[2]}/${m[1]}`:new Date(v).toLocaleDateString('pt-BR');};
const dateTimeBr=(v:any)=>v?new Date(v).toLocaleString('pt-BR'):'—';
const cpfBr=(v:any)=>{const d=String(v??'').replace(/\D/g,'');return d.length===11?`${d.slice(0,3)}.${d.slice(3,6)}.${d.slice(6,9)}-${d.slice(9)}`:(v??'—');};
const phoneBr=(v:any)=>{const d=String(v??'').replace(/\D/g,'');if(d.length===11)return`(${d.slice(0,2)}) ${d.slice(2,7)}-${d.slice(7)}`;if(d.length===10)return`(${d.slice(0,2)}) ${d.slice(2,6)}-${d.slice(6)}`;return v||'—';};
...
export function EmployeePortalPage(){
  ...
  async function loadProfile(){setProfileBusy(true);try{const r=await api.myProfile();setProfile(r.employee);}catch(e){setError(e instanceof Error?e.message:'Falha ao carregar perfil.');}finally{setProfileBusy(false);}}
  async function openProfile(){setProfileOpen(true);if(!profile)await loadProfile();}
  ...
  useEffect(()=>{
    ...
    const onMessage=(event:MessageEvent)=>{
      if(event.data?.type==='PAYHUB_NOTIFICATION_CLICK'||event.data?.type==='PAYHUB_REFRESH_PAYROLLS')refresh();
    };
    window.addEventListener('focus',onFocus);
    window.addEventListener('payhub:refresh-payrolls',onPortalRefresh);
    document.addEventListener('visibilitychange',onVisibility);
    navigator.serviceWorker?.addEventListener?.('message',onMessage);
    return()=>{
      window.removeEventListener('focus',onFocus);
      window.removeEventListener('payhub:refresh-payrolls',onPortalRefresh);
  ...
  return <main className="employee-portal"><header className="employee-header"><div className="employee-header-main"><Brand subtitle="Portal do funcionário"/></div><div className="employee-header-actions"><NotificationCenter/><button className="employee-profile-trigger" aria-label="Abrir meu perfil" onClick={()=>void openProfile()}><span className="avatar">{principal?.name.slice(0,2).toUpperCase()}</span><span className="employee-profile-trigger-copy"><strong>{principal?.name.split(' ')[0]}</strong><small>Meu perfil</small></span><span className="employee-profile-chevron">›</span></button></div></header><section className="employee-content"><div className="employee-hero"><div><span className="eyebrow light">MEUS DOCUMENTOS</span><h1>Olá, {principal?.name.split(' ')[0]}.</h1><p>Acompanhe seus holerites e conclua assinaturas pendentes.</p></div><div className="hero-stat"><strong>{rows.filter((r)=>['SIGNATURE_REQUESTED','VIEWED'].includes(r.status)).length}</strong><span>pendente(s) de assinatura</span></div></div><div className="employee-toolbar"><div className="search-box"><span>⌕</span><input placeholder="Filtrar por competência, ex.: 09/2026" value={filter} onChange={(e)=>setFilter(e.target.value)}/></div><button className="secondary-button compact" onClick={exportReport}>Relatório CSV</button></div>{error&&<div className="form-error">{error}</div>}<div className="payroll-card-grid">{filtered.map((p)=><article className={`employee-payroll-card ${p.status==='SIGNED'?'signed':''}`} key={p.id}><div className="payroll-card-top"><div className="month-badge"><strong>{String(p.month).padStart(2,'0')}</strong><span>{p.year}</span></div><StatusBadge status={p.status}/></div><h3>{p.payrollTypeLabel}</h3><div className="employee-money"><span>Valor líquido</span><strong>{money(p.netAmount)}</strong></div><div className="employee-payroll-actions"><button className="secondary-button compact" onClick={()=>void open(p.id)}>Visualizar completo</button>{p.status!=='SIGNED'?<button className="primary-button compact" onClick={async()=>{try{const r=await api.myPayroll(p.id);setSigning(r.payroll);}catch(e){setError(e instanceof Error?e.message:'Falha');}}}>Visualizar e assinar</button>:<button className="primary-button compact" disabled={downloadBusy===p.id} onClick={()=>void preparePayrollDownload(p)}>{downloadBusy===p.id?'Preparando PDF…':'Baixar / compartilhar'}</button>}</div>{p.signedAt&&<span className="signed-note">✓ Assinado em {new Date(p.signedAt).toLocaleString('pt-BR')}</span>}</article>)}</div>{filtered.length===0&&<div className="empty-state panel">Nenhum holerite disponível.</div>}</section>{detail&&<EmployeePayrollModal data={detail} onClose={()=>setDetail(null)} onDownload={preparePayrollDownload} busy={downloadBusy===detail.id}/>} {signing&&<SignModal data={signing} onClose={()=>setSigning(null)} onSigned={async()=>{setSigning(null);setDetail(null);await load();}}/>}{preparedDownload&&<Modal title="Holerite pronto" onClose={()=>setPreparedDownload(null)}><div className="form-section"><span className="eyebrow">PDF ASSINADO</span><h3>{preparedDownload.typeLabel} · {preparedDownload.competence}</h3><p className="muted">No iPhone, use <strong>Compartilhar / Salvar no iPhone</strong> para abrir a folha de compartilhamento do iOS. Nela você pode escolher “Salvar em Arquivos”, AirDrop, WhatsApp, e-mail e outros aplicativos.</p></div><div className="modal-actions"><button className="secondary-button" onClick={downloadPrepared}>Baixar PDF</button><button className="primary-button" onClick={()=>void sharePrepared()}>Compartilhar / Salvar no iPhone</button></div></Modal>}{profileOpen&&<EmployeeProfileModal profile={profile} busy={profileBusy} onRefresh={loadProfile} onClose={()=>setProfileOpen(false)} onLogout={()=>void logout()}/>}</main>;
}

function EmployeeProfileModal({profile,busy,onRefresh,onClose,onLogout}:{profile:any;busy:boolean;onRefresh():Promise<void>;onClose():void;onLogout():void}){
  return <Modal title="Meu perfil" onClose={onClose} wide>{busy&&!profile?<div className="employee-profile-loading">Carregando seus dados…</div>:profile?<div className="employee-self-profile"><div className="employee-self-profile-head"><div className="avatar employee-profile-avatar">{String(profile.name??'--').slice(0,2).toUpperCase()}</div><div className="employee-self-profile-name"><span className="eyebrow">PERFIL DO FUNCIONÁRIO</span><h2>{profile.name}</h2><p>{profile.jobTitle||'Função não informada'} · Matrícula Sage #{profile.sageEmployeeCode}</p></div><span className={`employee-profile-status ${profile.status==='ACTIVE'?'active':''}`}>{profile.status==='ACTIVE'?'Ativo':profile.status}</span></div><div className="employee-profile-stats"><div><strong>{profile.totalPayrolls}</strong><span>Holerites</span></div><div><strong>{profile.signedPayrolls}</strong><span>Assinados</span></div><div><strong>{profile.pendingPayrolls}</strong><span>Pendentes</span></div></div><div className="employee-profile-section"><span className="eyebrow">DADOS CADASTRAIS</span><div className="employee-profile-grid"><ProfileField label="Matrícula Sage" value={profile.sageEmployeeCode}/><ProfileField label="CPF" value={cpfBr(profile.cpf)}/><ProfileField label="Data de nascimento" value={dateBr(profile.birthDate)}/><ProfileField label="Admissão" value={dateBr(profile.admissionDate)}/><ProfileField label="Função / Cargo" value={profile.jobTitle||'—'}/><ProfileField label="CBO" value={profile.cbo||'—'}/><ProfileField label="Telefone" value={phoneBr(profile.phone)}/><ProfileField label="Grupo" value={profile.groupName||'—'}/><ProfileField label="Situação Sage" value={profile.sageStatus||'—'}/><ProfileField label="Situação PayHub" value={profile.status==='ACTIVE'?'Ativo':profile.status}/><ProfileField label="Acesso" value={profile.hasPin?'PIN cadastrado':'Primeiro acesso pendente'}/><ProfileField label="Último acesso" value={dateTimeBr(profile.lastLoginAt)}/></div></div><div className="employee-profile-source-note"><span>↻</span><p>Os dados cadastrais são sincronizados do Sage. Alterações de função, CBO, admissão e demais dados de origem devem ser realizadas no sistema de folha e sincronizadas com o PayHub.</p></div><div className="employee-profile-actions"><button className="secondary-button" disabled={busy} onClick={()=>void onRefresh()}>{busy?'Atualizando…':'Atualizar perfil'}</button><button className="employee-logout-profile" onClick={onLogout}>Sair da conta</button></div></div>:<div className="form-error">Não foi possível carregar o perfil.</div>}</Modal>
}
function ProfileField({label,value}:{label:string;value:any}){return <div className="employee-profile-field"><span>{label}</span><strong>{value??'—'}</strong></div>}
