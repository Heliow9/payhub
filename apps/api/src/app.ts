...
import { ConnectorAuthError, ConnectorService, ConnectorValidationError } from './domain/connectors/connector.service.js';
import type { ConnectorRepository } from './domain/connectors/connector.repository.js';
import type { ImportJobRepository } from './domain/connectors/import-job.repository.js';
...
import { createConnectorsRouter } from './http/routes/connectors.routes.js';
import { createImportJobsRouter } from './http/routes/import-jobs.routes.js';
import { createConnectorAgentRouter } from './http/routes/connector-agent.routes.js';
...
export function createApp(deps: AppDependencies) {
  ...
  const connectorService = new ConnectorService(deps.connectors, deps.importJobs, deps.audit);
  ...
  app.use('/api/connectors', createConnectorsRouter(auth, connectorService));
  app.use('/api/import-jobs', createImportJobsRouter(auth, connectorService));
  app.use('/api/connector-agent', createConnectorAgentRouter(connectorService));
