...
export interface PayHubServices {
  ...
  runs: PayrollRunService; payrolls: PayrollService; settings: SettingsService; signatures: SignatureService; storage: StorageService;
}

export function createServices(pool:Pool,env:Env):PayHubServices{
  const audit=new AuditService(pool);const auth=new AuthService(pool,env,audit);const connector=new ConnectorService(pool,audit);const storage=new StorageService(env.DOCUMENT_STORAGE_PATH);const settings=new SettingsService(pool,audit);const employees=new EmployeeService(pool,connector,audit);const groups=new GroupService(pool,audit);const runs=new PayrollRunService(pool,connector,audit);const payrolls=new PayrollService(pool,storage,audit);const signatures=new SignatureService(pool,storage,settings,audit,env);return{audit,auth,connector,employees,groups,runs,payrolls,settings,signatures,storage};
}
