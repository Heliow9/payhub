...
public sealed class Worker : BackgroundService
{
    ...
    public Worker(ILogger<Worker> logger, PayHubApiClient api, SageReadOnlyClient sage, IOptions<PayHubOptions> options)
    {
        _logger = logger; _api = api; _sage = sage; _options = options.Value;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception ex) { _logger.LogError(ex, "Falha no ciclo do conector."); }
    ...
    private async Task SendHeartbeatIfDueAsync(CancellationToken cancellationToken)
    {
        ...
        await _api.HeartbeatAsync(Environment.MachineName, new
        {
            ...
            connectorVersion = typeof(Worker).Assembly.GetName().Version?.ToString() ?? "3.0.0"
        }, cancellationToken);
    ...
    private async Task ExecuteJobAsync(ImportJobDto job, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            switch (job.JobType)
            {
                case "CONNECTION_TEST": await RunConnectionTestAsync(job, cancellationToken); break;
                case "SCHEMA_DISCOVERY": await RunSchemaDiscoveryAsync(job, cancellationToken); break;
                case "PAYROLL_IMPORT": await RunPayrollImportAsync(job, cancellationToken); break;
                case "EMPLOYEE_LOOKUP_BY_CPF": await RunEmployeeLookupAsync(job, cancellationToken); break;
                default: throw new InvalidOperationException($"Tipo de job não suportado: {job.JobType}");
            }
        }
        catch (Exception ex)
        {
            ...
            catch (Exception reportError) { _logger.LogError(reportError, "Não foi possível reportar a falha do job {JobId} ao PayHub.", job.Id); }
        }
    }
    ...
    private async Task RunConnectionTestAsync(ImportJobDto job, CancellationToken cancellationToken)
    {
        ...
        await _api.SendBatchAsync(job.Id, "ConnectionTest", 0, [row], cancellationToken);
    ...
    private async Task RunSchemaDiscoveryAsync(ImportJobDto job, CancellationToken cancellationToken)
    {
        ...
        var rows = schema.Select(item => new Dictionary<string, object?> { ["tableName"] = item.TableName, ["columns"] = item.Columns, ["rowCount"] = item.RowCount }).ToList();
        await _api.SendBatchAsync(job.Id, "SchemaDiscovery", 0, rows, cancellationToken);
    ...
    private async Task RunEmployeeLookupAsync(ImportJobDto job, CancellationToken cancellationToken)
    {
        var scope = DeserializeScope<EmployeeLookupScope>(job, "EMPLOYEE_LOOKUP_BY_CPF");
        if (scope.CompanyCode.ValueKind is JsonValueKind.Null or JsonValueKind.Undefined) throw new InvalidOperationException("companyCode é obrigatório.");
        await _api.ProgressAsync(job.Id, 0, 1, "Buscando funcionário pelo CPF no Sage…", cancellationToken);
        var employee = await _sage.LookupEmployeeByCpfAsync(scope, cancellationToken);
        var rows = employee is null ? new List<Dictionary<string, object?>>() : [employee];
        await _api.SendBatchAsync(job.Id, "EmployeeLookup", 0, rows, cancellationToken);
        await _api.ProgressAsync(job.Id, 1, 1, employee is null ? "CPF não localizado no Sage." : "Funcionário localizado no Sage.", cancellationToken);
        await _api.CompleteAsync(job.Id, employee is null ? "Funcionário não encontrado no Sage." : "Funcionário encontrado no Sage.", cancellationToken);
    }

    private async Task RunPayrollImportAsync(ImportJobDto job, CancellationToken cancellationToken)
    {
        var scope = DeserializeScope<PayrollImportScope>(job, "PAYROLL_IMPORT");
        if (scope.CompanyCode.ValueKind is JsonValueKind.Undefined or JsonValueKind.Null) throw new InvalidOperationException("companyCode é obrigatório para PAYROLL_IMPORT.");
        var tableIndex = 0; var batches = 0; string? currentTable = null;
        ...
        await foreach (var batch in _sage.ReadPayrollAsync(scope, cancellationToken))
        {
            ...
            if (!string.Equals(currentTable, batch.Table, StringComparison.OrdinalIgnoreCase)) { currentTable = batch.Table; tableIndex++; }
            await _api.SendBatchAsync(job.Id, batch.Table, batch.BatchNumber, batch.Rows, cancellationToken); batches++;
    ...
    private static T DeserializeScope<T>(ImportJobDto job, string type)
    {
        if (!job.Scope.HasValue || job.Scope.Value.ValueKind != JsonValueKind.Object) throw new InvalidOperationException($"{type} sem escopo válido.");
        return JsonSerializer.Deserialize<T>(job.Scope.Value.GetRawText(), new JsonSerializerOptions(JsonSerializerDefaults.Web) { PropertyNameCaseInsensitive = true })
               ?? throw new InvalidOperationException($"Não foi possível interpretar o escopo de {type}.");
    }
}
